...
- `optical-recorder` recorder service
...
Install resources live in `extra/`:

- `optical-archive-recorder.service`: systemd unit for optical-recorder.
...
Run from the deployed `scripts` directory on the target node:
...
sudo /opt/burnbridge/scripts/optical-archive-service.sh restart
sudo /opt/burnbridge/scripts/optical-archive-service.sh logs
...
- `OpticalArchive__LinuxServices__MountRefreshServiceType=command` makes optical-recorder use the configured command template.
...
- `MountRefreshServiceType=direct` keeps optical-recorder on the built-in `umount` + `mount -o ro` path.
- If the JSON shared config also defines `OpticalArchive:LinuxServices`, restart optical-recorder after changing either the JSON config or these environment overrides.
