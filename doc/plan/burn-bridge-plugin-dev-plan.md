# VersityGW Burn Bridge 插件集成开发计划（PrimoBurner BlockDevice 流式刻录）

## 1. 项目背景与目标

### 1.1 背景
当前目标是将 VersityGW 作为 S3 网关前端，通过插件后端将对象数据桥接到 .NET Core gRPC 刻录服务，最终由 PrimoBurner SDK 的 BlockDevice 能力实现光盘流式刻录。

目标数据链路：

`S3 Client -> VersityGW -> Burn Bridge Plugin(Go) -> gRPC Service(.NET Core) -> PrimoBurner BlockDevice -> Optical Drive`

### 1.2 目标
- 在不修改 VersityGW 核心 S3 API 行为的前提下，以插件方式接入刻录能力。
...
### 1.3 范围
**In Scope**
- Go 插件后端开发（实现 `plugins.BackendPlugin`，返回 `backend.Backend` 实例）。
...
### 2.1 VersityGW 插件接入点
- 使用 `versitygw plugin <plugin.so> --config <config>` 加载后端插件。
- 插件导出 `var Backend plugins.BackendPlugin`。
- `Backend.New(config)` 返回实现 `backend.Backend` 的对象。
...
3. **幂等与恢复**
   ...
   - 插件崩溃恢复后可通过 gRPC `GetJobStatus` 回补状态。

4. **流控策略**
   - 插件侧配置 chunk size、窗口大小、发送超时。
...
| M2 插件骨架+协议联通 | W2 | 插件可被加载，gRPC 基础调用可用 | plugin skeleton、healthcheck、proto v1 | `versitygw plugin` 启动成功，health OK |
...
| T-005 | 插件框架 | 实现 BackendPlugin.New/config 解析 | 插件可加载 | T-004 | P0 | S | 1.5 | Go | 未开始 |
| T-006 | 插件框架 | Backend 基类（嵌入 BackendUnsupported） | 基础后端对象 | T-005 | P0 | S | 1 | Go | 未开始 |
...
| T-017 | 恢复能力 | 插件重启后任务状态恢复 | recovery 机制 | T-016 | P1 | L | 3 | Go | 未开始 |
...
| T-005 | 插件入口与配置 | Go | 2026-05-15 | 2026-05-16 |  | 未开始 | 0 | 低 |  | 创建插件 skeleton |  |
...
|---|---|---|---|---|---|---|---|
| R-001 | Go plugin 与主程序构建参数不一致导致运行异常 | 中 | 高 | 高 | 固定 Go 版本+构建镜像+CI 强校验 | Go负责人 | 打开 |
...
## 9. 文件与维护建议
- 本计划文件：`doc/plan/burn-bridge-plugin-dev-plan.md`
- 建议新增：
